...
- Add typed event map and `emit/on` helpers.
- Replace raw `CustomEvent` casts in generation/result flows.
- Remove dead `SETTINGS_UPDATED` usage or implement real subscribers.
- Add output escaping helper for dynamic `innerHTML` text interpolation.

Deliverable: stable event contracts and reduced runtime fragility.

## Phase 2 - Generation Use Case Extraction

- Create `GenerateSvgUseCase` for provider/key resolution, fallback policy, sanitization, and result shaping.
- Reduce `src/index.ts` to orchestration adapter only.
- Move UI notifications/navigation side effects behind small adapter interfaces.
