...
function renderGallery(svgs: GalleryItem[]) {
  ...
  if (svgs.length === 0) {
    svgGrid = `
      <div class="flex flex-col items-center justify-center py-24 bg-surface rounded-2xl border border-border mt-8">
    ...
    svgs.forEach((item) => {
      svgGrid += `
        <div class="flex flex-col bg-surface border border-border rounded-xl overflow-hidden hover:border-border-bright transition-all">
          <div class="flex-1 flex items-center justify-center p-8 bg-surface-hover/10 relative min-h-[220px]">
              ${item.svg.replace(/<svg\b([^>]*)>/i, '<svg class="max-h-32 w-full" $1>')}
          </div>
          <div class="px-5 py-4 border-t border-border flex flex-col gap-3">
            ...
            <div class="flex justify-between items-center">
              ...
              <div class="flex gap-1">
                ...
                <button data-action="delete-svg" data-id="${item.id}" class="p-1.5 rounded-md text-text-secondary hover:text-error hover:bg-red-500/10 transition-all" title="Delete">
